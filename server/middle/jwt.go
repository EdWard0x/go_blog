...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// 检查Access Token是否在黑名单中，如果是，则清除Refresh Token并返回未授权错误
		...
		// 检查Refresh Token是否在黑名单中，如果是，则清除Refresh Token并返回未授权错误||清除了缓存，refresh token失效，判断是否为空
		if jwtService.IsInBlacklist(refreshToken) || refreshToken == "" {
			utils.ClearRefreshToken(c)
			response.NoAuth("Account logged in from another location or token is invalid", c)
			c.Abort() // 终止请求的后续处理
			return
