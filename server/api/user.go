...
func (userApi *UserApi) QQLogin(c *gin.Context) {
	...
	// 获取访问令牌
	accessTokenResponse, err := service.ServiceGroupApp.GetAccessTokenByCode(code)
...
func (userApi *UserApi) TokenNext(c *gin.Context, user database.User) {
	...
	// 添加以下日志输出
	//global.Log.Info("Refresh Token Expiration Debug",
	//	zap.Time("refresh_claims_expires_at", refreshClaims.ExpiresAt.Time),
	//	zap.Int64("refresh_claims_expires_at_unix", refreshClaims.ExpiresAt.Unix()),
	//	zap.Int64("current_time_unix", time.Now().Unix()),
	//	zap.Int("calculated_max_age_seconds", int(refreshClaims.ExpiresAt.Unix()-time.Now().Unix())),
	//	zap.Any("时间:", int(refreshClaims.ExpiresAt.Unix()-time.Now().Unix())),
	//)
	...
	if jwtStr, err := service.ServiceGroupApp.GetRedisJWT(user.UUID); errors.Is(err, redis.Nil) {
		// 不存在就设置新的
		if err := service.ServiceGroupApp.SetRedisJWT(refreshToken, user.UUID); err != nil {
		...
		if err := service.ServiceGroupApp.JoinInBlacklist(blacklist); err != nil {
