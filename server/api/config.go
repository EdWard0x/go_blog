...
// GetCF 获取CF配置
func (configApi *ConfigApi) GetCF(c *gin.Context) {
	response.OkWithData(global.Config.CF, c)
}

// UpdateCF 更新CF配置
func (configApi *ConfigApi) UpdateCF(c *gin.Context) {
	var req config.Cloudflare
	err := c.ShouldBindJSON(&req)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	...
	err = configService.UpdateCF(req)
	if err != nil {
		global.Log.Error("Failed to update CF:", zap.Error(err))
		response.FailWithMessage("Failed to update CF", c)
		return
	}
	response.OkWithMessage("Successfully updated CF", c)
}
