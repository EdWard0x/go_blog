...
func ElasticsearchImport(jsonPath string) (int, error) {
	...
	indexExists, err := esService.IndexExists(es.ArticleIndex())
	if err != nil {
		return 0, err
	}
	if indexExists {
		if err := esService.IndexDelete(es.ArticleIndex()); err != nil {
			return 0, err
		}
	}
	err = esService.IndexCreate(es.ArticleIndex(), es.ArticleMapping())
	if err != nil {
		return 0, err
	}
