...
func (c *Comment) AfterCreate(_ *gorm.DB) error {
	...
	_, err := global.ESClient.Update(es.ArticleIndex(), c.ArticleID).Script(&script).Do(context.TODO())
	return err
}
...
func (c *Comment) BeforeDelete(_ *gorm.DB) error {
	...
	source := "ctx._source.comments -= 1"
	script := types.Script{Source: &source, Lang: &scriptlanguage.Painless}
	_, err := global.ESClient.Update(es.ArticleIndex(), articleID).Script(&script).Do(context.TODO())
	return err
}
