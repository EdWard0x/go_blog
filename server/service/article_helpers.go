...
func (articleService *ArticleService) Create(a *es.Article) error {
	// 将文章索引到Elasticsearch中，并设置刷新操作为 true
	_, err := global.ESClient.Index(es.ArticleIndex()).Request(a).Refresh(refresh.True).Do(context.TODO())
	return err
}
...
func (articleService *ArticleService) Delete(ids []string) error {
	...
	_, err := global.ESClient.Bulk().Request(&request).Index(es.ArticleIndex()).Refresh(refresh.True).Do(context.TODO())
	return err
}

// Get 用于通过ID从 Elasticsearch 获取文章
func (articleService *ArticleService) Get(id string) (es.Article, error) {
	var a es.Article
	// 从Elasticsearch获取文章
	res, err := global.ESClient.Get(es.ArticleIndex(), id).Do(context.TODO())
	if err != nil {
		return es.Article{}, err
	}
	// 如果找不到该文档，则返回错误
	if !res.Found {
		return es.Article{}, errors.New("document not found")
	}
...
func (articleService *ArticleService) Update(articleID string, v any) error {
	...
	_, err = global.ESClient.Update(es.ArticleIndex(), articleID).Request(&update.Request{Doc: bytes}).Refresh(refresh.True).Do(context.TODO())
	return err
}
...
func (articleService *ArticleService) Exits(title string) (bool, error) {
	...
	// 执行搜索查询，查找是否存在该标题的文章
	res, err := global.ESClient.Search().Index(es.ArticleIndex()).Request(req).Size(1).Do(context.TODO())
	if err != nil {
		return false, err
	}
	// 如果存在该标题，返回 true
