...
func (articleService *ArticleService) ArticleInfoByID(id string) (es.Article, error) {
	// 异步更新浏览量
	go func() {
...
func (articleService *ArticleService) ArticleLike(req request.ArticleLike) error {
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		_, err := global.ESClient.Update(es.ArticleIndex(), req.ArticleID).Script(&script).Do(context.TODO())
		return err
	})
}
...
func (articleService *ArticleService) ArticleLikesList(info request.ArticleLikesList) (interface{}, int64, error) {
	...
	var list []struct {
		Id_     string     `json:"_id"`
		Source_ es.Article `json:"_source"`
	}
