...
func (userService *UserService) Logout(c *gin.Context) {
	...
	accessToken := utils.GetAccessToken(c)    // 获取Access Token
	refreshToken := utils.GetRefreshToken(c)  // 获取Refresh Token
	
	utils.ClearRefreshToken(c)
	global.Redis.Del(context.Background(), uuid.String())
	
	// 将Refresh Token加入黑名单
	_ = ServiceGroupApp.JwtService.JoinInBlacklist(database.JwtBlacklist{Jwt: refreshToken})
	
	// 将Access Token也加入黑名单
	if accessToken != "" {
		_ = ServiceGroupApp.JwtService.JoinInBlacklist(database.JwtBlacklist{Jwt: accessToken})
	}
}
...
func (userService *UserService) UserWeather(ip string) (string, error) {
	...
	result, err := global.Redis.Get(context.Background(), "weather-"+ip).Result()
	if err != nil {
		...
		// 将天气数据存入redis
		if err := global.Redis.Set(context.Background(), "weather-"+ip, weather, time.Hour*1).Err(); err != nil {
			return "", err
		}
		return weather, nil
	}
